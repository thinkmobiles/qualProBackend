var CONSTANTS = require('../constants/mainConstants');

module.exports = (function () {
    var mongoose = require('mongoose');
    var ObjectId = mongoose.Schema.Types.ObjectId;


    /**
     * @constructor OutletModel
     * @type {*|Schema}
     *
     * @property {String} name _name_ of _Outlet_
     * @property {String} description _description_ of _Outlet_
     * @property {ObjectId} country ___reference___ to {@link CountryModel}
     * @property {ObjectId} manager ___reference___ to {@link PersonnelModel}
     * @property {Array} branches array of ___reference___ to {@link OutletModel}
     * @property {String} imageSrc ___base64___ representation of outlet image
     * @property {Bool} isOwn _flag if Outlet_ is own or it is competitors outlet
     * @property {Bool} isArchived _flag if _Personnel_ is archived or active
     * @property {String} firstName _First name_ of _Personnel_
     */
    var schema = mongoose.Schema({
        name: {type: String, default: 'empty'},
        description: {type: String, default: ''},
        country: {type: ObjectId, ref: CONSTANTS.COUNTRY, default: null},
        manager: {type: ObjectId, ref: CONSTANTS.PERSONNEL, default: null},
        branches: {type: [ObjectId], ref: CONSTANTS.BRANCH, default: null},
        imageSrc: {
            type: String,
            default: 'data:image/png;base64,'
        },
        isOwn: {type: Boolean, default: true},
        isArchived: Boolean,
        createdBy: {
            user: {type: ObjectId, ref: CONSTANTS.PERSONNEL, default: null},
            date: {type: Date, default: Date.now}
        },
        editedBy: {
            user: {type: ObjectId, ref: CONSTANTS.PERSONNEL, default: null},
            date: {type: Date}
        }

    }, {collection: 'outlets'});

    mongoose.model(CONSTANTS.OUTLET, schema);

    if (!mongoose.Schemas) {
        mongoose.Schemas = {};
    }

    mongoose.Schemas[CONSTANTS.OUTLET] = schema;
})();

